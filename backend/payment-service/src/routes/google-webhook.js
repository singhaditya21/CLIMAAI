 ...
 *
 * A factory rather than a bare module-level router so tests can hand in their
 * own env and a stub OAuth2Client: the real one can only say yes to a token
 * Google actually signed, which is exactly the thing a forgery test cannot
 * have.
 */
function createGoogleWebhookRouter({ env = process.env, authClient = new OAuth2Client() } = {}) {
    ...
    const PUBSUB_AUDIENCE = env.PUBSUB_AUDIENCE || '';
    const PUBSUB_SERVICE_ACCOUNT_EMAIL = env.PUBSUB_SERVICE_ACCOUNT_EMAIL || '';
    const EXPECTED_PACKAGE_NAME = env.GOOGLE_PACKAGE_NAME || '';
...
module.exports = createGoogleWebhookRouter();
module.exports.createGoogleWebhookRouter = createGoogleWebhookRouter;
module.exports.PushVerificationError = PushVerificationError;
